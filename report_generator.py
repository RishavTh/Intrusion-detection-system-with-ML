...
class ReportTemplate:
    ...
    def on_page(self, canvas, doc):
        ...
        canvas.drawString(15*mm, 3.5*mm,
            f'Linux Authentication IDS  |  Confidential Security Report  |  {datetime.now().strftime("%d %B %Y")}')
        canvas.setFillColor(colors.white)
    ...
    def on_first_page(self, canvas, doc):
        ...
        canvas.drawString(15*mm, h - 14*mm,
            'Linux Authentication Intrusion Detection System  |  Confidential')
        ...
        canvas.setFont('Helvetica', 9)
        canvas.setFillColor(colors.HexColor('#8ab4d4'))
        canvas.drawCentredString(w/2, h/2 - 15*mm, 'Prepared by: Linux Auth IDS — Automated Security Report')
        canvas.drawCentredString(w/2, h/2 - 22*mm, 'Classification: CONFIDENTIAL — Internal Use Only')
        canvas.drawCentredString(w/2, h/2 - 29*mm, f'Report Generated: {datetime.now().strftime("%d %B %Y at %H:%M")}')
        canvas.drawCentredString(w/2, h/2 - 36*mm, f'System Host: {socket.gethostname()}  |  Ubuntu 24.04 LTS')
...
def generate_report(output_path):
    ...
    story.append(Paragraph(
        f'The Linux Authentication IDS has been monitoring <b>{socket.gethostname()}</b> (Ubuntu 24.04 LTS, '
    ...
    sys_data = [
        ...
        [Paragraph('Host', styles['tbl_cell']),
         Paragraph(socket.gethostname(), styles['tbl_cell']),
        ...
        [Paragraph('Report Generated', styles['tbl_cell']),
         ...
         Paragraph('Linux Auth IDS — Automated Analysis', styles['tbl_cell'])],
    ]
    ...
    story.append(Paragraph(
        ...
        'Report classification: CONFIDENTIAL — For internal use only — Linux Authentication IDS.</i>',
